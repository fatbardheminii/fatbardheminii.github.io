...
userInput.addEventListener("keydown", function (event) {
  ...
  //if there isn't a number in expression but we press one of the operators, than we need to insert the first
  // character to 0 before the operator, so the function eval than will still be valid
  else if(operators.includes(pressedKey) &&
    expression.length < 1) {
      userInput.value = 0 + expression.substring(0, expression.length);
    }
  ...
  //if last character of expression is a dot and then we press an operator we MUST delete the dot and display the operator
  // otherwise the function eval won't work
   else if (
    operators.includes(pressedKey) &&
    dotKeydownEvent.includes(expression.charAt(expression.length - 1))
  ) {
    userInput.value = expression.substring(0, expression.length - 1);
    /*if next to last character of expression is operator AND
    if last character is dot and we again press an operator--> than delete last 2 characters(opreator+dot) and
    display only the last pressed operator*/
    if(operators.includes(expression.charAt(expression.length - 2))) {
      userInput.value = expression.substring(0, expression.length -2);
    }
  }
  ...
  /*if expression from 0 to first substring includes a dot == TRUE
    ...
    operatorIndexes.length < 1) {
        ...
        /* the following else if checks for the case when, the expression includes a dot and its length is greater than 15 than
        it prevents other numbers to be displayed.. THIS is very important because the next else if is only valid for the expression 
        without dots!!*/
        else if(expression.substring(0, operatorIndexes[0]).length > 15) {
          if(!operators.includes(pressedKey)) {
            event.preventDefault();
            console.log('dot is there and only 16 numbers max until the first operator is displayed');
          }
        }
  }
  //MAX 16 NUMBERS WITHOUT OPERATOR CAN BE DISPLAYED
    else if(
    expression.substring(0, operatorIndexes[0]).length > 15 &&
    operatorIndexes.length < 1) {
        if(!operators.includes(pressedKey)) {
            event.preventDefault();
            console.log("16 numbers is max");
        }
  } 
  ...
  //MAX 16 NUMBERS FROM LAST OPERATOR TO THE END CAN BE DISPLAYED
    else if(
    expression.substring(operatorIndexes[operatorIndexes.length - 1], expression.length).length > 16
  ) {
    if(!operators.includes(pressedKey)) {
      event.preventDefault();
      console.log("max 16 numbers after operator");
...
//THIS IS THE PART ONLY WHEN BUTTONS ARE CLICKED
